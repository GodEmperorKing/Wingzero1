...
output "instance_external_ips" {
  value = {
    vm1 = "http://${google_compute_instance.iowa1-vm.network_interface[0].access_config[0].nat_ip}"
    vm2 = "http://${google_compute_instance.hongkong1-vm.network_interface[0].access_config[0].nat_ip}"
    vm3 = "http://${google_compute_instance.frankfurt1-vm.network_interface[0].access_config[0].nat_ip}"
 }
